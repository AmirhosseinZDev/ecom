...
class LoginTicketFlowITest extends AbstractIntegrationITest {
    ...
    void registered_user_logs_in_with_otp_and_receives_a_working_jwt() throws Exception {
        ...
        postJson("/api/user/login-ticket", Map.of("mobileNumber", mobile))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.ticketTTLInSecond").isNumber());

        String otp = captureLastOtp();
        MvcResult validation = postJson("/api/user/login-ticket/validation",
    ...
    void login_ticket_for_unregistered_mobile_is_rejected_without_sms() throws Exception {
        postJson("/api/user/login-ticket", Map.of("mobileNumber", newMobile()))
    ...
    void validate_login_ticket_with_wrong_otp_is_rejected() throws Exception {
        String mobile = newMobile();
        register(mobile);
        postJson("/api/user/login-ticket", Map.of("mobileNumber", mobile)).andExpect(status().isOk());

        postJson("/api/user/login-ticket/validation", Map.of("mobileNumber", mobile, "ticket", "000000"))
    ...
    void login_ticket_with_malformed_mobile_is_rejected() throws Exception {
        postJson("/api/user/login-ticket", Map.of("mobileNumber", "0912"))
                .andExpect(status().isBadRequest());
        assertEquals(0, smsRequestCount());
    }
    ...
    void check_registration_reflects_user_state() throws Exception {
        ...
        MvcResult before = postJson("/api/user/check-registration", Map.of("mobileNumber", mobile))
                .andExpect(status().isOk())
                .andReturn();
        ...
        MvcResult after = postJson("/api/user/check-registration", Map.of("mobileNumber", mobile))
                .andExpect(status().isOk())
                .andReturn();
