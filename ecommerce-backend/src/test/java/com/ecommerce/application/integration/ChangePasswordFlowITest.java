...
class ChangePasswordFlowITest extends AbstractIntegrationITest {
    ...
    void change_password_rotates_credential() throws Exception {
        ...
        postJson("/api/user/login", Map.of("mobileNumber", mobile, "password", DEFAULT_PASSWORD))
                .andExpect(status().isUnauthorized());
        postJson("/api/user/login", Map.of("mobileNumber", mobile, "password", NEW_PASSWORD))
                .andExpect(status().isOk());
    }
    ...
    void change_password_with_blank_fields_is_rejected() throws Exception {
        ...
        postJson("/api/user/change-password", Map.of("newPassword", "", "confirmPassword", ""), token)
                .andExpect(status().isBadRequest());
    }
